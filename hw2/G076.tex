...
\begin{document}
    ...
    \begin{questions}
        ...
        \begin{equation*}
             \text{median}(\{x^{(i)}_{3}\}_{i = 1}^{10}) = 2.5
        \end{equation*}
        ...
        \begin{itemize}
            ...
            \begin{equation*}
                H(t) = - \sum_{i = 0}^{1}\frac{\#\{k|t^{(k)} = i\}}{8} \log_2\left(\frac{\#\{k|t^{(j)} = i\}}{8}\right) = - \left(\frac{1}{2}\log_2\left(\frac{1}{2}\right) + \frac{1}{2}\log_2\left(\frac{1}{2}\right)\right) = 1 \medspace bit
            \end{equation*}
        ...
        \begin{table}[H]
            ...
            \end{tabular}
        \end{table}
        Partition $(y_1 = 2)$ has no uncertainty and we cannot reduce uncertainty in partition $(y_1 = 0)$ (note that there is only one value of $y_2$ (resp. $y_3$), so $H(t|y_2, y_1 = 1) = H(t|y_1 = 1)$ (resp., $H(t|y_3, y_1 = 1)$), so there is no possible information gain). We thus proceed to divide partition $(y_1 = 1)$ even further in an analogous way as before: 
        \begin{itemize}
            \item Starting entropy:
        \end{itemize}
        \vspace{-1.75em}
        \begin{flalign*}
            ...
            &= - \left(\frac{3}{5}\log_2\left(\frac{3}{4}\right) + \frac{1}{4}\log_2\left(\frac{1}{4}\right)\right) = 0.688721 \medspace bit
        \end{flalign*}
        \vspace{-1.5em}
        \begin{itemize}
            \item Variable conditional entropies and corresponding information gains (where $\text{IG}(y_i|y_1 = 1) = H(t|y_1 = 1) - H(t|y_i, y_1 = 1))$:
        \end{itemize}
        \vspace{-1em}
        \begin{gather*}
            H(t|y_2, y_1 = 1) = \sum_{i = 0}^{2} \frac{\#\{k|y_2^{(k)} = i \wedge y_1^{(k)} = 1\}}{\#\{k|y_1^{(k)} = 1)\}}H(t|y_2 = i, y_1 = 1) \\
                = - \sum_{i = 0}^{2}  \frac{\#\{k|y_2^{(k)} = i \wedge y_1^{(k)} = 1)\}}{\#\{k|y_1^{(k)} = 1\}} \sum_{j = 0}^{1}  \frac{\#\{k|t^{(k)} = j \wedge y_2^{(k)} = i \wedge y_1^{(k) } = 1)\}}{\#\{k|y_2^{(k)} = i \wedge y_1^{(k)} = 1\}} \log_2\left(\frac{\#\{k|y_2^{(k)} = i \wedge y_1^{(k)} = 1)\}}{\#\{k|y_1^{(k)} = 1\}}\right) \\
        ...
        \end{alignat*}
        As $y_3$ provides the biggest information gain, it will originate a new partition in the partition $y_1 = 1$. The following divided dataset cannot be further partitioned ($(y_1 = 1, y_3 = 1)$ has no uncertainty and we can't obtain information gain from dividing $(y_1 = 1, y_3 = 0)$ for the same argument as the applied above to $(y_1 = 0)$):
        \begin{table}[H]
            \centering
            ...
            \end{tabular}
        \end{table}
        And thus we have the desired decision tree:
        \vspace{-0.25em}
        \begin{table}[H]
            \centering
        ...
        \begin{bmatrix}
            1 & 2 & 1
        \end{bmatrix}^T$, we have that $\textbf{\^{t}}(\textbf{x}^{(10)}) = \text{N}$. Since $\textbf{t}(\textbf{x}^{(9)}) = \text{N}$ and $\textbf{t}(\textbf{x}^{(10)}) = \text{P}$ there are no true positives and no true negatives in the classification of these instances. Thus, accuracy on the testing data is 0 according to the formula $\text{Accuracy} = \frac{TP + TN}{P + N}$.
    ...
    \begin{questions}
        ...
        \begin{parts}
            ...
            \begin{figure}[H]
                \centering
                \subfloat{{\includegraphics[scale = 0.47]{accuracy_n_features.pdf} }}%
                \qquad
                \subfloat{{\includegraphics[scale = 0.47]{accuracy_n_depths.pdf} }}
            \end{figure}
        \end{parts}
    \end{questions}
