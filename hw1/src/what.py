...
dataset[dataset.columns[-1]] = dataset[dataset.columns[-1]].str.decode('utf8') dataset = dataset.dropna() #remove instances with missing features
return dataset
def plot_grid(dataset):
fig, axs = plt.subplots(3, 3)
bins = np.linspace(1, 11, 11) # create bins from 1 to 10
for (index, feature) in zip([(i, j) for i in range(0, 3) for j in range(0, 3)], dataset.columns[:-
1]):
...
axs[index[0], index[1]].set_yticks(np.linspace(0, 1, 5)) axs[index[0],index[1]].tick_params(axis = 'both', labelsize = 8)
...
kf = model_selection.KFold(n_splits = 10, shuffle = True, random_state = 76) fig, ax = plt.subplots()
test_errors = {}
for k in (3, 5, 7):
...
plot_grid(dataset)
kNN_cross_validation(inputs=dataset.iloc[:,:-1].values, outputs=dataset[dataset.columns[-1]].values) test_kNN_NBayes(inputs=dataset.iloc[:,:-1].values, outputs=dataset[dataset.columns[-1]].values)