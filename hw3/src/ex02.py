...
def mlp_predict(mlp_model ,inputs, outputs, folds, early_stp, alpha):
    clf = mlp_model(activation = 'relu',    hidden_layer_sizes = (3, 2), early_stopping = early_stp,\
                                            alpha = alpha, random_state = 76, max_iter = 1500)
    return model_selection.cross_val_predict(clf, inputs, outputs, cv = folds)

def mlp_conf_matrix(inputs, outputs, folds, early_stp):
    outputs_pred = mlp_predict(neural_network.MLPClassifier, inputs, outputs, folds, early_stp, 0.4)
    disp = metrics.ConfusionMatrixDisplay.from_predictions(outputs, outputs_pred)
    disp.ax_.set(title=f'Confusion Matrix (with early stopping)' if early_stp else \
                       f'Confusion Matrix (without early stopping)')
    plt.savefig(f"output/mlp_conf_matrix_{early_stp}.pdf")

def residue_dist_bp(inputs, outputs, folds):
    res_regularized = outputs - mlp_predict(neural_network.MLPRegressor, \
                                            inputs, outputs, folds, True, 4)
    res_nonregularized = outputs - mlp_predict(neural_network.MLPRegressor, \
                                            inputs, outputs, folds, True, 0)
    fig, ax = plt.subplots()
    bp = ax.boxplot([res_regularized, res_nonregularized], \
                    vert = False, flierprops={'marker': 'o', 'markersize': 2})
    ax.set_yticklabels(['without regularization','with $\\ell_2$ regularization \n ($\\alpha = 4$)'])
